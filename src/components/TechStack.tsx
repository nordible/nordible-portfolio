...
export default function TechStack() {
  ...
  return (
    <section className="relative py-24 bg-gray-50 dark:bg-gray-900/80 overflow-hidden">
      ...
      <div className="relative w-full">
        <div className="text-center mb-16 px-4 sm:px-6 lg:px-8">
          <div className="inline-block px-4 py-1 mb-6 text-[10px] font-mono font-bold tracking-[0.3em] text-gray-500 uppercase">
            Engineering Arsenal
          </div>
          <h2 className="text-4xl sm:text-5xl font-bold text-gray-900 dark:text-white mb-6 tracking-tight">
            Our Technology <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-orange-600">Stack</span>
          </h2>
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between space-x-4 sm:space-x-8 overflow-hidden py-10">
            {getVisibleTechnologies().map((tech, index) => (
              <div 
                key={`${tech.name}-${index}`}
                className="tech-card rounded-xl p-6 transition-all duration-500 hover:scale-110 border border-gray-100 dark:border-gray-800 group flex-shrink-0 w-28 h-28 sm:w-40 sm:h-40 flex flex-col items-center justify-center relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-white dark:bg-gray-800 opacity-50 group-hover:opacity-100 transition-opacity"></div>
                <img 
                  src={tech.logo} 
                  alt={tech.name}
                  ...
                  onError={(e) => {
                    ...
                    if (parent) {
                      ...
                      div.className = "h-16 w-16 bg-gradient-to-br from-purple-500 to-orange-500 rounded-lg flex items-center justify-center text-white font-mono font-bold text-2xl mb-4 shadow-xl z-10";
                      div.textContent = tech.name.charAt(0);
                      parent.appendChild(div);
                    }
                  }}
                />
                <span className="font-mono text-[10px] sm:text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest z-10 group-hover:text-purple-500 transition-colors">{tech.name}</span>
        ...
        <div className="max-w-4xl mx-auto mt-16 px-4 sm:px-6 lg:px-8">
          <div className="tech-card rounded-2xl p-8 border border-purple-500/20 relative overflow-hidden">
            <div className="absolute top-0 left-0 w-1 h-full bg-purple-500"></div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center uppercase tracking-wider">
              <span className="font-mono text-purple-500 mr-3">_manifest</span>
